...
fn help_exits_successfully_and_lists_every_group() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        (
            output.status.code(),
            stderr,
            stdout.contains("--group register-p6|register-p5|vm|concurrency|tui"),
...
fn unknown_group_lists_every_configured_group() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ...
        (
            Some(1),
            String::new(),
            "error: no benchmarks in group `missing` (known groups: register-p6, register-p5, vm, concurrency, tui)".to_owned()
        )
    );
    Ok(())
}
