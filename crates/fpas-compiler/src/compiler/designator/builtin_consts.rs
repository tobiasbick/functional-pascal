...
use fpas_std::{
    CM_ABOUT, CM_CANCEL, CM_CLOSE, CM_OK, CM_OPEN, CM_QUIT, CM_USER, MESSAGE_BOX_OPTION_ABOUT,
...
impl Compiler {
    pub(super) fn builtin_const_value(name: &str) -> Option<Value> {
        match name {
            ...
            s::STD_TUI_CM_OK => Some(Value::Integer(CM_OK)),
            s::STD_TUI_CM_CANCEL => Some(Value::Integer(CM_CANCEL)),
            s::STD_TUI_CM_CLOSE => Some(Value::Integer(CM_CLOSE)),
            s::STD_TUI_CM_QUIT => Some(Value::Integer(CM_QUIT)),
