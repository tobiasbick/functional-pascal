    ...
    pub(super) fn compile_tui_application_call(
        ...
        match name {
            ...
            s::STD_TUI_APPLICATION_CHECKED => {
                self.expect_exact_args(s::STD_TUI_APPLICATION_CHECKED, 2, args, location)?;
                for arg in args {
                    self.compile_expr(arg)?;
                }
                self.emit_intrinsic(Intrinsic::Tui(TuiIntrinsic::Checked), location);
                Ok(true)
            }
            s::STD_TUI_APPLICATION_SELECTED => {
                self.expect_exact_args(s::STD_TUI_APPLICATION_SELECTED, 2, args, location)?;
                for arg in args {
                    self.compile_expr(arg)?;
                }
