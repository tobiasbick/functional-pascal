...
impl Compiler {
    /// Lower Turbo Vision `Std.Tui` calls to VM intrinsics.
    pub(super) fn compile_tui_bridge_call(
        &mut self,
        name: &str,
        args: &[Expr],
        ...
        match name {
            ...
            s::STD_TUI_TEST_INJECT_COMMAND => {
                ...
                self.emit_intrinsic_unit(Intrinsic::Tui(TuiIntrinsic::TestInjectCommand), location);
                Ok(true)
            }
