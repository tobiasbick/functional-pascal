...
#[test]
fn std_tui_list_selection_lowers_to_intrinsic() {
    let chunk = compile_ok(
        "\
program T;
uses Std.Tui;
...
begin
  ...
  var Selection: integer := Application.ListSelection(App, ListHandle)
    ...
    assert!(
        chunk.code().iter().any(
            |op| matches!(op, Op::Intrinsic(code) if *code == u16::from(Intrinsic::Tui(TuiIntrinsic::ListSelection)))
        ),
        "expected Application.ListSelection intrinsic in generated bytecode"
    );
}
