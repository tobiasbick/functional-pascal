//! DAP task-handle assignment mapping and invalidation coverage.
...
fn server() -> DapServer {
    ...
    let executable = fpas_compiler::compile(&program).expect("compile DAP task-handle fixture");
...
fn jsonl_server() -> JsonlServer {
    let (program, diagnostics) = fpas_parser::parse(SOURCE);
    assert!(diagnostics.is_empty(), "parse diagnostics: {diagnostics:?}");
    let executable = fpas_compiler::compile(&program).expect("compile JSONL task-handle fixture");
...
fn stop_with_initialized_locals(adapter: &mut DapServer, seq: &mut u64) -> u64 {
    ...
    panic!("DAP task-handle fixture locals never became initialized")
}
...
fn initialize_and_stop_jsonl(server: &mut JsonlServer) -> (u64, u64) {
    ...
    panic!("JSONL task-handle fixture locals never became initialized")
}

#[test]
fn dap_set_variable_and_set_expression_copy_task_handles() {
    let mut adapter = server();
    let (mut seq, initial_frame) = initialize_and_stop(&mut adapter, true);
    let pending = evaluate(&mut adapter, &mut seq, initial_frame, "Pending");
    assert_eq!(pending[0]["success"], true, "{pending:?}");
    let pending_value = pending[0]["body"]["result"].as_str().expect("pending");
    assert!(pending_value.starts_with("<task "), "{pending_value}");

    let textual = send(
        &mut adapter,
        &mut seq,
        "setExpression",
        json!({"frameId":initial_frame,"expression":"Current","value":"Pending"}),
    );
    assert_eq!(textual.len(), 2, "{textual:?}");
    assert_eq!(textual[0]["success"], true);
    assert_eq!(textual[0]["body"]["value"], pending_value);
    ...
    let handle = send(
        ...
        &mut seq,
        "setVariable",
        json!({"variablesReference":locals,"name":"Current","value":"Pending"}),
    );
    ...
    let rejected = send(
        ...
        &mut seq,
        "setExpression",
        json!({"frameId":current,"expression":"Frozen","value":"Pending"}),
    );
...
#[test]
fn dap_omits_task_assignment_invalidation_without_client_support() {
    let mut adapter = server();
...
#[test]
fn dap_and_jsonl_task_assignment_results_and_errors_match() {
    let mut dap = server();
    ...
    let dap_success = send(
        ...
        &mut dap_seq,
        "setExpression",
        json!({"frameId":dap_frame,"expression":"Current","value":"Pending"}),
    );
    let jsonl_success = jsonl_send(
        &mut jsonl,
        &mut jsonl_id,
        "expression.set",
        json!({"frame_id":initial_jsonl_frame,"target":"Current","expression":"Pending"}),
    );
    assert_eq!(
    ...
    let dap_failure = send(
        ...
        &mut dap_seq,
        "setExpression",
        json!({"frameId":dap_frame,"expression":"Current","value":"1"}),
    );
    let jsonl_failure = jsonl_send(
        &mut jsonl,
        &mut jsonl_id,
        "expression.set",
        json!({"frame_id":current_jsonl_frame,"target":"Current","expression":"1"}),
    );
    assert_eq!(
