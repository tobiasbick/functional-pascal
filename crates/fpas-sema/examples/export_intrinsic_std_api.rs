...
fn render_constants(
    ...
    output.push_str("\npublic const\n");
    for name in names {
        let symbol = symbols.remove(&name).expect("catalog constant");
...
fn render_types(
    ...
    for name in type_names {
        let symbol = symbols.remove(&name).expect("catalog type");
        render_documentation(output, unit, &name, symbol, documentation, "  ");
        match &symbol.ty {
...
#[expect(
    ...
    reason = "record rendering keeps catalog inputs explicit"
)]
fn render_record(
    output: &mut String,
    unit: &str,
    ...
    for member_name in member_names {
        let symbol = symbols.remove(member_name).expect("catalog record member");
