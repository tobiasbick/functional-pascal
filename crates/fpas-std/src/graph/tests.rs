...
use super::stub::run_graph_intrinsic;
...
fn application_open_reports_not_implemented_yet() {
    let err = run_graph_intrinsic(GraphIntrinsic::ApplicationOpen, test_location())
        .expect_err("expected stub error");
    assert!(
...
fn graph_session_upload_frame_rejects_surface_size_mismatch() {
    ...
    assert!(
        error.message.contains("expected Width=2 and Height=2, got Width=3 and Height=2"),
        "message={}",
        error.message
    );
}
...
fn graph_session_size_and_close_follow_open_state() {
    ...
    session.close(test_location()).expect("close should succeed");
    let error = session
        .size(test_location())
...
fn graph_event_kind_matches_payload_variant() {
    assert_eq!(GraphEvent::CloseRequested.kind(), GraphEventKind::CloseRequested);
    assert_eq!(
        GraphEvent::Resize {
            width: 640,
            height: 480,
