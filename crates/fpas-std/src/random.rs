...
pub(crate) fn run(
    ...
    match intrinsic {
        Intrinsic::Random(RandomIntrinsic::Random) => {
            ...
            call.push(Value::Real(random_real));
        }
        Intrinsic::Random(RandomIntrinsic::RandomInt) => {
            let upper_bound = pop_int(pop_value(call, location)?, location)?;
            let lower_bound = pop_int(pop_value(call, location)?, location)?;
            ...
            call.push(Value::Integer(random_integer));
        }
        Intrinsic::Random(RandomIntrinsic::Randomize) => {
            call.push(Value::Unit);
        }
        _ => return Ok(None),
    }
    Ok(Some(()))
