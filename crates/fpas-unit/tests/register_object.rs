...
use fpas_unit::object::{
    ObjectError, REGISTER_OBJECT_VERSION, RelocatableObject, RelocationKind, decode_object,
    encode_object,
};
...
fn conversion_covers_every_register_table_operand_and_is_deterministic() {
    ...
    assert_eq!(first.version, REGISTER_OBJECT_VERSION);
...
fn incompatible_object_version_is_rejected_before_linking() {
    ...
    object.version = REGISTER_OBJECT_VERSION - 1;
    assert_eq!(
        object.validate(),
        Err(ObjectError::Version {
            actual: REGISTER_OBJECT_VERSION - 1,
...
#[test]
fn every_truncated_register_object_prefix_is_rejected_without_panic() {
