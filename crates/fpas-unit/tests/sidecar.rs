...
use fpas_unit::{
    ...
    InvalidationReason, SidecarLoad, UnitIdentity, encode, load_sidecar, sidecar_path,
    write_sidecar,
};
...
fn written_sidecar_is_reusable() {
    ...
    assert_eq!(loaded, SidecarLoad::Reusable(Box::new(unit)));
    fs::remove_dir_all(dir).ok();
}
...
fn replacing_sidecar_keeps_a_complete_valid_object() {
    ...
    assert_eq!(loaded, SidecarLoad::Reusable(Box::new(replacement)));
    fs::remove_dir_all(dir).ok();
}
...
fn concurrent_writers_publish_one_complete_object() {
    ...
    assert!(unit.object == b"first" || unit.object == b"second");
    fs::remove_dir_all(dir).ok();
}
