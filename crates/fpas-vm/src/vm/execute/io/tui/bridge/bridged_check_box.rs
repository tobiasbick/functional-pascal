//! Turbo Vision bridge modal `CheckBox` view that mirrors checked state into the FPAS host cell.
//!
...
impl BridgedCheckBox {
    ...
    /// Copy upstream checkbox state into the host cell (Turbo Vision read-back path).
    pub(in crate::vm::execute::io::tui) fn sync_checked_from_view(&mut self) {
        self.sync_checked();
    }

    /// Push FPAS cell state into the upstream checkbox (Turbo Vision setter path).
