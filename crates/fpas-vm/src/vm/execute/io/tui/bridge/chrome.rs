//! Turbo Vision bridge menu bar and status line chrome (`MenuBar.New`, `Application.SetMenuBar`, …).
//!
...
use crate::vm::execute::io::tui::bridge::session::{TuiMenuBarState, TuiStatusLineState};
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_menu_bar_new(
    worker: &mut Worker,
    bounds: Rect,
    ...
    let handle = worker.bridge.registry.allocate(0, ViewKind::MenuBar);
    worker.bridge.insert_menu_bar(
        handle,
        TuiMenuBarState {
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_status_line_new(
    worker: &mut Worker,
    bounds: Rect,
    ...
    let handle = worker.bridge.registry.allocate(0, ViewKind::StatusLine);
    worker.bridge.insert_status_line(
        handle,
        TuiStatusLineState {
            bounds: state_rect(bounds),
            items,
...
pub(in crate::vm::execute::io::tui) fn bridge_set_menu_bar(
    worker: &mut Worker,
    menu_bar_handle: u32,
    line: SourceLocation,
) -> Result<(), VmError> {
    ...
    if worker.bridge.attached_menu_bar().is_some() {
        return Err(runtime_error(
            RUNTIME_INTRINSIC_STACK_STATE_ERROR,
            "Application already has a menu bar on the Turbo Vision path",
            "Call `Application.SetMenuBar` only once per session.",
            line,
        ));
    }

    worker.bridge.set_attached_menu_bar(menu_bar_handle);
    bridge_sync_chrome_to_app(worker, line)
}

/// Attaches a status line to the application session (`Application.SetStatusLine`).
pub(in crate::vm::execute::io::tui) fn bridge_set_status_line(
    worker: &mut Worker,
    ...
    if worker.bridge.attached_status_line().is_some() {
        return Err(runtime_error(
            RUNTIME_INTRINSIC_STACK_STATE_ERROR,
            "Application already has a status line on the Turbo Vision path",
            "Call `Application.SetStatusLine` only once per session.",
            line,
        ));
    }

    worker.bridge.set_attached_status_line(status_line_handle);
    bridge_sync_chrome_to_app(worker, line)
}

/// Replaces menu bar menus (`MenuBar.SetMenus`).
pub(in crate::vm::execute::io::tui::bridge) fn bridge_menu_bar_set_menus(
    worker: &mut Worker,
    menu_bar_handle: u32,
    menus: Vec<TurboVisionMenu>,
    line: SourceLocation,
    ...
    if worker.bridge.attached_menu_bar() == Some(menu_bar_handle) {
        bridge_sync_chrome_to_app(worker, line)?;
    }
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_status_line_set_items(
    worker: &mut Worker,
    status_line_handle: u32,
    items: Vec<TurboVisionStatusItem>,
    line: SourceLocation,
    ...
    if worker.bridge.attached_status_line() == Some(status_line_handle) {
        bridge_sync_chrome_to_app(worker, line)?;
    }
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_sync_chrome_to_app(
    worker: &mut Worker,
    line: SourceLocation,
) -> Result<(), VmError> {
    if worker.bridge.attached_menu_bar().is_none() && worker.bridge.attached_status_line().is_none()
    {
        return Ok(());
    }
    ...
    let menu_bar = worker
        ...
        .map(|snapshot| build_bridge_menu_bar(snapshot.bounds, &snapshot.menus));
    let status_line = worker
        .bridge
        .attached_status_line_snapshot()
        .map(|snapshot| build_bridge_status_line(snapshot.bounds, &snapshot.items));
...
fn build_bridge_menu_bar(
    bounds: crate::vm::shared::TurboVisionRect,
    menus: &[TurboVisionMenu],
) -> MenuBar {
    let mut menu_bar = MenuBar::new(turbo_rect(bounds));
    for menu in menus {
        menu_bar.add_submenu(SubMenu::new(&menu.title, build_bridge_menu(menu)));
    }
    menu_bar
}

fn build_bridge_menu(menu: &TurboVisionMenu) -> Menu {
    Menu::from_items(
        menu.items
            .iter()
            .map(|item| {
...
fn bridge_session_closed_error(line: SourceLocation) -> VmError {
    runtime_error(
        ...
        "Call `Application.Open` before creating Turbo Vision chrome on the Turbo Vision path.",
        line,
    )
}
...
mod tests {
    ...
    #[test]
    fn bridge_menu_commands_keep_upstream_ids() {
        let menu = build_bridge_menu(&TurboVisionMenu {
            title: "~F~ile".into(),
            items: vec![
                TurboVisionMenuItem {
                    text: "~O~pen".into(),
    ...
    #[test]
    fn bridge_status_commands_keep_upstream_ids() {
        let status_line = build_bridge_status_line(
            rect(),
    ...
    fn set_menus_updates_attached_chrome_snapshot() {
        ...
        worker.bridge.open();
        let handle = bridge_menu_bar_new(
            &mut worker,
            Rect::new(0, 0, 60, 1),
            vec![TurboVisionMenu {
                title: "OLD".into(),
        ...
        .expect("menu bar");
        bridge_set_menu_bar(&mut worker, handle, loc()).expect("attach");
        bridge_menu_bar_set_menus(
            &mut worker,
            handle,
            vec![TurboVisionMenu {
                title: "NEW".into(),
