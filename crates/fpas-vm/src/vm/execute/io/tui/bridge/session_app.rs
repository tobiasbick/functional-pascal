//! Turbo Vision bridge live turbo-vision `Application` session owned for one FPAS `Application.Open` … `Close` cycle.
//!
//! The session lives on the main [`Worker`](crate::vm::Worker) only (`!Send`). Turbo Vision bridge modals and
