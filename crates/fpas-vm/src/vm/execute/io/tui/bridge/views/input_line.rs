//! Turbo Vision bridge `InputLine` construction, attach, and read-back.
use crate::vm::Worker;
use crate::vm::diagnostics::{VmError, runtime_error};
use crate::vm::execute::io::tui::bridge::registry::{RegistryError, ViewKind};
use crate::vm::execute::io::tui::bridge::session::{DetachedInputLine, TuiRoot};
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_input_line_new(
    worker: &mut Worker,
    bounds: Rect,
    text: String,
    max_length: usize,
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_input_line_text(
    worker: &mut Worker,
    handle: u32,
    line: SourceLocation,
    ...
    worker.bridge.commit_input_line_text(handle);
    Ok(worker
        .bridge
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_input_line_set_text(
    worker: &mut Worker,
    handle: u32,
    text: String,
    line: SourceLocation,
    ...
    let Some(cell) = worker.bridge.input_line_cell(handle) else {
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_dialog_attach_input_line(
    worker: &mut Worker,
    dialog_handle: u32,
    ...
    let Some(DetachedInputLine {
        ...
        text_cell,
        max_length,
    }) = worker.bridge.take_detached_input_line(input_line_handle)
    else {
        return Err(runtime_error(
            RUNTIME_INTRINSIC_STACK_STATE_ERROR,
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_window_attach_input_line(
    worker: &mut Worker,
    window_handle: u32,
    input_line_handle: u32,
    line: SourceLocation,
...
fn missing_input_line_cell(handle: u32, line: SourceLocation) -> VmError {
    runtime_error(
        ...
        "Use a handle returned by `InputLine.New` in the active Turbo Vision session.",
        line,
    )
}
...
mod tests {
    ...
    fn input_line_new_registers_handle() {
        ...
        worker.bridge.open();
        let handle = bridge_input_line_new(
            &mut worker,
            Rect::new(2, 1, 18, 1),
            "FPAS".into(),
            32,
        ...
        assert_eq!(
            bridge_input_line_text(&mut worker, handle, loc()).unwrap(),
            "FPAS"
        );
    }
    ...
    fn set_text_updates_host_cell() {
        ...
        worker.bridge.open();
        let handle =
            bridge_input_line_new(&mut worker, Rect::new(2, 1, 18, 1), "old".into(), 32, loc())
                .expect("input line");
        bridge_input_line_set_text(&mut worker, handle, "new".into(), loc()).expect("set text");
        assert_eq!(
            bridge_input_line_text(&mut worker, handle, loc()).unwrap(),
            "new"
        );
    ...
    fn dialog_add_input_line_registers_upstream_view_id() {
        ...
        let dialog =
            bridge_dialog_new_modal(&mut worker, Rect::new(5, 3, 30, 8), "Test".into(), loc())
                .expect("dialog");
        let input_line = bridge_input_line_new(
            &mut worker,
            Rect::new(4, 2, 18, 1),
            "FPAS".into(),
            32,
        ...
        .expect("input line");
        bridge_dialog_attach_input_line(&mut worker, dialog, input_line, loc()).expect("attach");
        let entry = worker
            .bridge
            .registry
