//! Turbo Vision bridge `StaticText` construction, parent attachment, and `SetText`.
//!
...
use crate::vm::execute::io::tui::bridge::session::{DetachedStaticText, TuiRoot};
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_static_text_new(
    worker: &mut Worker,
    bounds: Rect,
    text: String,
    line: SourceLocation,
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_static_text_set_text(
    worker: &mut Worker,
    handle: u32,
    text: String,
    line: SourceLocation,
    ...
    worker.bridge.set_static_text_text(handle, text.clone());

    if worker.bridge.child_parent(handle).is_some() {
        let bounds = bridge_with_child_view(worker, handle, ViewKind::StaticText, line, |view| {
            Ok(view.bounds())
        })?;
    ...
    } else if let Some(bounds) = worker.bridge.detached_static_text_bounds(handle) {
        worker
            .bridge
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_dialog_attach_static_text(
    worker: &mut Worker,
    dialog_handle: u32,
    ...
    let Some(DetachedStaticText {
        static_text,
        local_bounds: _,
    }) = worker.bridge.take_detached_static_text(static_text_handle)
    else {
...
pub(in crate::vm::execute::io::tui::bridge) fn bridge_window_attach_static_text(
    worker: &mut Worker,
    window_handle: u32,
    static_text_handle: u32,
    line: SourceLocation,
...
mod tests {
    ...
    use crate::vm::execute::io::tui::bridge::views::window::bridge_window_new;
    ...
    fn static_text_new_registers_handle() {
        ...
        worker.bridge.open();
        let handle =
            bridge_static_text_new(&mut worker, Rect::new(2, 1, 20, 1), "Label".into(), loc())
                .expect("static text");
        assert_eq!(
            worker
                .bridge
    ...
    fn set_text_updates_host_state() {
        ...
        worker.bridge.open();
        let handle =
            bridge_static_text_new(&mut worker, Rect::new(3, 2, 20, 1), "OLD".into(), loc())
                .expect("static text");
        bridge_static_text_set_text(&mut worker, handle, "NEW".into(), loc()).expect("set text");
        assert_eq!(worker.bridge.static_text_text(handle), Some("NEW"));
    }
    ...
    fn window_add_static_text_registers_upstream_view_id() {
        ...
        let window = bridge_window_new(&mut worker, Rect::new(5, 3, 30, 10), "Test".into(), loc())
            .expect("window");
        let label =
            bridge_static_text_new(&mut worker, Rect::new(4, 2, 20, 1), "Hello".into(), loc())
                .expect("label");
        bridge_window_attach_static_text(&mut worker, window, label, loc()).expect("attach");
        let entry = worker
            .bridge
            .registry
    ...
    fn dialog_add_static_text_registers_upstream_view_id() {
        ...
        let label =
            bridge_static_text_new(&mut worker, Rect::new(4, 2, 20, 1), "Hello".into(), loc())
                .expect("label");
        bridge_dialog_attach_static_text(&mut worker, dialog, label, loc()).expect("attach");
        let entry = worker
            .bridge
            .registry
            .require(label, ViewKind::StaticText)
