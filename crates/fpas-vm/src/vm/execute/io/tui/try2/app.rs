//! Try-2 live turbo-vision `Application` session (no try-1 snapshot populate).
//!
...
/// Creates or reuses the live turbo-vision application for try-2 (empty desktop).
pub(in crate::vm::execute::io::tui::try2) fn try2_ensure_live_app(
    worker: &mut Worker,
    line: SourceLocation,
) -> Result<(), VmError> {
