//! Try-2 command callback dispatch (upstream `CM_*` ids without offset translation).
//!
...
#[must_use]
pub(in crate::vm::execute::io::tui::try2) fn try2_command_for_callback(command: CommandId) -> i64 {
    i64::from(command)
}

/// Invokes the registered `OnCommand` handler with the upstream command id unchanged.
pub(in crate::vm::execute::io::tui::try2) fn try2_dispatch_on_command(
    worker: &mut Worker,
    command: CommandId,
    line: SourceLocation,
) -> Result<(), VmError> {
    ...
    let Some(handler) = handler else {
        return Err(runtime_error(
            ...
            "Application.Run requires an `Application.OnCommand` handler on the try-2 path",
            "Call `Application.OnCommand(App, OnCommand)` before `Application.Run(App)`.",
            line,
        ));
    };
    try2_dispatch_on_command_handler(worker, &handler, command, line)
}

/// Invokes a specific `OnCommand` handler (used when the handler is passed at run time later).
pub(in crate::vm::execute::io::tui::try2) fn try2_dispatch_on_command_handler(
    worker: &mut Worker,
    handler: &Value,
    command: CommandId,
    line: SourceLocation,
    ...
    let _ = worker.call_function_sync_allowing_shutdown(
        handler,
        &[app_rec, Value::Integer(try2_command_for_callback(command))],
        line,
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn command_ids_pass_through_unchanged() {
        assert_eq!(try2_command_for_callback(CM_OK), 10);
        assert_eq!(try2_command_for_callback(CM_QUIT), 1);
        assert_eq!(try2_command_for_callback(24), 24);
        assert_eq!(try2_command_for_callback(4096), 4096);
    }
}
