//! Try-2 `Application.MessageBox` on the live or headless turbo-vision session.
//!
...
/// Shows a message box when the try-2 session is active.
pub(in crate::vm::execute::io::tui) fn try2_message_box(
    worker: &mut Worker,
    message: String,
    options: u16,
    line: SourceLocation,
    ...
    let command = if worker.with_tui(|tui| tui.session.is_headless()) {
        try2_headless_message_box(worker, &message, options, line)?
    } else {
...
fn try2_headless_message_box(
    ...
    let command = try2_headless_exec_view(worker, dialog, line)?;
