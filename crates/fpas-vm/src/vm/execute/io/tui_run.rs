    ...
    fn prepare_tui_application_run(&mut self, line: SourceLocation) -> Result<(), VmError> {
        ...
        if !has_global_paint && !has_retained_paint {
            return Err(runtime_error(
                ...
                "Call `Application.HostRegisterOnPaint(App, OnPaint)`, `Application.HostRegisterOnViewPaint(App, ViewId, OnViewPaint)`, `Application.HostCreateSolidFillView(...)`, or `Application.HostCreateStatusBarView(...)` before `Application.Run(App)`.",
                line,
            ));
        }
