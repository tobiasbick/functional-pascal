//! Dense global and positional aggregate operations for the register VM.

use fpas_bytecode::{
    AbcOperands, AbxOperands, SharedPositionalEnum, SharedPositionalRecord, Value,
};
...
impl RegisterWorker {
    ...
    pub fn make_record(&mut self, o: AbcOperands) -> Result<(), VmError> {
        ...
        self.write(
            register(o.a)?,
            Value::PositionalRecord(SharedPositionalRecord::new(layout, values)),
        )
    }
    ...
    pub fn load_field(&mut self, o: AbcOperands) -> Result<(), VmError> {
        let value = match self.read(register(o.b)?)? {
            Value::Record(record) => record
                ...
                .get(usize::from(o.c))
                .map(|(_, value)| value.clone()),
            Value::PositionalRecord(record) => record.body().values.get(usize::from(o.c)).cloned(),
        ...
        let record = match self.read(register(o.a)?)?.clone() {
            Value::Record(mut record) => {
                record
                    ...
                    .ok_or_else(|| self.bad_slot("record field", u32::from(o.b)))?
                    .1 = value;
                Value::Record(record)
            }
            Value::PositionalRecord(mut record) => {
                *record
                    .values_mut()
                    .get_mut(usize::from(o.b))
    ...
    pub fn update_record(&mut self, o: AbcOperands) -> Result<(), VmError> {
        ...
        if !matches!(record, Value::Record(_) | Value::PositionalRecord(_)) {
    ...
    pub fn make_enum(&mut self, o: AbcOperands) -> Result<(), VmError> {
        ...
        self.write(
            register(o.a)?,
            Value::PositionalEnum(SharedPositionalEnum::new(variant, values)),
        )
    }
    ...
    pub fn test_variant(&mut self, o: AbcOperands) -> Result<(), VmError> {
        ...
        let matches = match self.read(register(o.b)?)? {
            Value::Enum(value) => {
                value.type_name == variant.type_name && value.variant == variant.variant
            }
    ...
    pub fn load_enum_field(&mut self, o: AbcOperands) -> Result<(), VmError> {
        let value = match self.read(register(o.b)?)? {
            Value::Enum(value) => value.fields.get(usize::from(o.c)).cloned(),
            Value::PositionalEnum(value) => value.body().values.get(usize::from(o.c)).cloned(),
            other => return Err(self.type_mismatch("enum", other)),
        }
