...
use crate::vm::execute::io::console_records::console_key_event_from_value;
...
impl RegisterWorker {
    pub(super) fn execute_graph_intrinsic(
        ...
        let result = match operation {
            ...
            GraphIntrinsic::ApplicationSize => {
                ...
                Some(records::size(width, height))
            }
    ...
    pub(super) fn with_graph<R>(
        &self,
        operation: impl FnOnce(&mut crate::vm::shared::GraphState) -> R,
    ) -> R {
        operation(
            &mut self
                .hosted
...
pub(super) fn application(value: Option<&Value>, worker: &RegisterWorker) -> Result<(), VmError> {
    match value {
        Some(Value::Record(record)) if record.type_name == "Std.Graph.Application" => Ok(()),
        actual => Err(worker.runtime_error(
            RUNTIME_VM_OPERAND_TYPE_MISMATCH,
            format!(
...
fn pixels(value: Option<&Value>, worker: &RegisterWorker) -> Result<Vec<i64>, VmError> {
