...
pub(super) use state::RegisterTaskState;
    ...
    pub(super) fn spawn_task(
        ...
        let target = function.function.ok_or_else(|| {
            diagnostics::at_address(
                ...
                RUNTIME_INVALID_TASK,
                format!(
                    "Function `{}` has no numeric register target",
                    function.name
                ),
                "Compile the task target with the register compiler.",
            )
        })?;
        let info = self
            .executable
        ...
        scheduler.enqueue(RegisterTaskState {
            id,
            function: target,
