...
use crate::vm::register::RegisterCallbackSession;
...
fn array_style_callbacks_use_numeric_targets_repeatedly() {
    let mut callbacks = RegisterCallbackSession::new(callback_image());
...
fn cancellation_and_shutdown_reject_later_callbacks() {
    let mut cancelled = RegisterCallbackSession::new(callback_image());
    cancelled.cancel();
    assert_eq!(
        cancelled
    ...
    let mut shutdown = RegisterCallbackSession::new(callback_image());
    shutdown.shutdown();
    assert_eq!(
        shutdown
            .invoke(FunctionId::new(1), vec![Value::Integer(1)])
