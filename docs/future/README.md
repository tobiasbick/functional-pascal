...
Implemented today: `Std.Console`, `Std.Args`, `Std.Env`, `Std.Path`, `Std.Graph`, `Std.Tui`, `Std.Str`, `Std.Conv`, `Std.Math`, `Std.Random`, `Std.Array`, `Std.Dict`, `Std.Result`, `Std.Option`, `Std.Task`. Planned additions (filesystem, time, JSON, parsing): [`std-roadmap.md`](std-roadmap.md).
