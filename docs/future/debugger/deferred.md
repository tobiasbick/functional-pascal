...
|---|---|---|---|
| DBG-D03 | Remaining identity-bearing assignment | Anonymous newly entered closures, copying already materialized task-bound functions, bound-receiver synthesis, `Dynamic` callable endpoints, capture cells as destinations, opaque resources, or in-place callable editing | Prove portable types, identity, escape, lifetime, ownership, and bounded atomic commit without display-data inference |
