...
| DBG-D04 | Control flow beyond bounded forced return | Root/task completion, non-active or multi-frame unwind, completed-return replacement, runtime-error recovery, arbitrary instruction changes, source-initializer suppression, and frame restart | Specify cleanup, scheduler and waiter effects, frame/task ownership, portable result proof, and rollback |
