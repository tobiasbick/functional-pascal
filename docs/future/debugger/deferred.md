...
|---|---|---|---|
| DBG-D03 | Remaining identity-bearing assignment | Anonymous newly entered closures, mutable `Cell`/`EnclosingCell` capture construction, task-bound functions, bound-receiver synthesis, `Dynamic` callable endpoints, capture cells, opaque resources, or in-place callable editing | Prove portable types, identity, escape, lifetime, ownership, and bounded atomic commit without display-data inference |
