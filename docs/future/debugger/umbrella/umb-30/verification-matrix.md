...
| `V30-05` | Runtime recovery compares the exact unconsumed failure and preserves the original diagnostic | VM failure/recovery tests | pass |
| `V30-06` | Consumed/stale failure and unsupported failure class cannot partially recover | VM negative tests | pass |
| `V30-07` | Retained completed-result replacement is exact and typed, or its missing invariant is recorded | Scheduler/VM and paired adapter/editor tests | pass |
| `V30-08` | Frame restart retains current args and exact captures, clears locals/temporaries, and removes younger frames | VM frame/register/capture tests | pass |
| `V30-09` | Restart executes no instruction; repeated side effects occur only after explicit continue | VM event/side-effect tests | pass |
| `V30-10` | Source-initializer suppression uses portable binding/store identity, never a heuristic | Metadata round-trip and VM tests or decision evidence | pass |
...
| `V30-14` | Current docs describe implemented behavior and limitations only | Documentation review | pass through U30-42 |
| `V30-15` | No FPAS syntax, semantic, or language-spec change | Diff review | pass through U30-42 |
| `V30-16` | Formatting, locked workspace build/tests, changed-library Clippy, extension tests, and diff check pass | Commands recorded in progress | pass through U30-42; final U30-60 closure pending |
