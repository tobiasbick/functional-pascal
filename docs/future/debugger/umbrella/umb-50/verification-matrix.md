...
|---|---|---|---|
| `V50-01` | Protocol stdout never contains raw debuggee bytes | Transport/adapter tests | pending |
| `V50-02` | Debuggee input is not parsed as JSONL or DAP | Negative framing tests | pending |
| `V50-03` | Disconnect and EOF clean up host handles without hidden execution | VM/host tests | pending |
| `V50-04` | Captured output remains bounded by the session output limit | Existing output-limit tests plus gaps | pending |
| `V50-05` | Live terminal input is ordered, cancellable, and reports EOF | VM/adapter tests or child-not-started evidence | pending |
...
| `V50-08` | JSONL and DAP expose equivalent I/O events and failures | Paired adapter tests | pending |
| `V50-09` | VS Code exposes only supported debuggee I/O | Extension-host tests | pending |
| `V50-10` | Current docs describe implemented behavior and limitations only | Documentation review | pending |
