...
|---|---|---|---|
| `V60-01` | Launch-owned sessions keep `attach` false until a proven subset exists | Capability and negative request tests | pass |
...
| `V60-06` | Native debugging is accepted only without a second semantic engine | `UMB-60C` rejection; disassemble/memory tests | pass |
| `V60-07` | JSONL and DAP expose equivalent attach success and failures | Paired adapter tests | pass |
| `V60-08` | VS Code exposes only supported attach UX | Extension unit test and launch-only manifest | pass |
| `V60-09` | Current docs describe implemented behavior and limitations only | Documentation review | pass |
