...
- Active work IDs: none; `U90-20` is pending
- Base checkpoint: `1a0e6c2e`
- Code changes after base: named accepted/rejected live-image classes;
  `reload.classify` / `fpas/reloadClassify` report them without replacement
- Next action: begin `U90-20` only after an explicit continuation request
...
- JSONL advertises `hot_reload: false` and `reload_classify: true`. DAP does
  not advertise hot reload. Named `reload` / `image.replace` / `fpas/reload`
  rejects change no stack. Classify names classes without applying them.
