...
| P4 Calls, frames, closures, and callbacks | complete | `p4-calls-closures.md`; 21 compiler register-subset tests, 20 direct register-VM tests, 21 register-bytecode verifier tests, and 19 IR validation tests; full fmt/build/workspace-test/all-target-clippy and FPAS fmt/test gates passed; production path unchanged |
| P5 Globals, records, enums, arrays, and dictionaries | complete | `p5-globals-aggregates.md`; compiler stack/register differential aggregate tests; direct register-VM aggregate/error tests; IR and bytecode malformed-layout tests; `cargo bench-fpas run --group register-p5` executes global, array, and record-update workloads entirely on the inactive register path; production path unchanged |
| P6 Intrinsics and hosted runtimes | complete | `p6-intrinsics-hosted-runtimes.md`; authoritative `Intrinsic::all()` plus explicit ID/uniqueness and canonical-catalog coverage; borrowed standard decoder; direct hosted Console/Test/headless Graph tests; numeric higher-order callback and intrinsic-loop differential tests; `register-p6-complete` release snapshot; P6-compatible 40-test register source corpus; full fmt/build/workspace-test/all-target-clippy and FPAS gates passed; production path unchanged |
