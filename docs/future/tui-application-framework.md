...
7. Revisit **performance**: double buffer, damage rectangles — Rust-internal, no FP contract change yet.
	...
	- ✅ Additional partial producer: `Application.HostRegisterView` and `Application.HostUnregisterView` now mark the affected view rectangle dirty, so host-managed view lifecycle changes contribute rect-based damage instead of relying on a later whole-frame redraw request. When a focused view is unregistered and focus falls back to another child, the newly focused view rectangle is dirtied as well. Key artifacts: [`tui.rs`](../../crates/fpas-std/src/tui.rs), [`tui/mod.rs`](../../crates/fpas-vm/src/vm/execute/io/tui/mod.rs), [`tui_host_vm.rs`](../../crates/fpas-vm/src/tests/core/tui_host_vm.rs).
	- ✅ Direct damage scheduling for resize: hosted resize dispatch now records redraw work in the damage tracker immediately, instead of relying on a separate follow-up request from `Application.Run`. This is still whole-frame invalidation today, but it keeps resize on the same explicit damage path as other redraw producers. Key artifacts: [`run_loop.rs`](../../crates/fpas-vm/src/vm/execute/io/tui/run_loop.rs), [`tui_run.rs`](../../crates/fpas-vm/src/vm/execute/io/tui_run.rs), [`tui_host_vm.rs`](../../crates/fpas-vm/src/tests/core/tui_host_vm.rs).
	- ✅ Startup damage preservation: `Application.Run` now requests its initial paint only when no redraw damage is pending yet, so startup no longer overwrites already accumulated damage information with an unconditional full-frame request. Key artifacts: [`tui.rs`](../../crates/fpas-std/src/tui.rs), [`tui_run.rs`](../../crates/fpas-vm/src/vm/execute/io/tui_run.rs).
	- ✅ Refined modal damage path: entering a modal frame without attached views stays redraw-free (state only), `Application.HostAttachViewToActiveModal` dirties the attached view rectangle, and `Application.HostLeaveModal` dirties the popped modal views plus any newly revealed underlying modal views. That keeps modal visibility changes on the explicit damage path without forcing a blanket full-frame redraw. Key artifacts: [`tui_modal.rs`](../../crates/fpas-std/src/tui_modal.rs), [`tui/mod.rs`](../../crates/fpas-vm/src/vm/execute/io/tui/mod.rs), [`tui_host_vm.rs`](../../crates/fpas-vm/src/tests/core/tui_host_vm.rs).
	- ✅ View-rect update producer: `Application.HostSetViewRect` now updates an existing host-managed view rectangle and dirties both the old and new bounds, so moving or resizing a registered view contributes explicit rect-based damage without going through a blanket redraw request. Key artifacts: [`tui/mod.rs`](../../crates/fpas-vm/src/vm/execute/io/tui/mod.rs), [`tui_focus.rs`](../../crates/fpas-compiler/src/tests/std_library/tui_focus.rs), [`tui_host_vm.rs`](../../crates/fpas-vm/src/tests/core/tui_host_vm.rs), [`tui-app.md`](../pascal/std/tui-app.md).
...
- Damage tracking is only partially exploited. Focus transitions, host-managed view registration/unregistration, and view-rect updates now emit partial dirty rectangles, including focus fallback on unregister; resize and startup now flow through the damage tracker directly, while modal attach/leave use view-based rect damage and bare modal enter remains state-only; mouse, paste, terminal focus, and most other host-state changes still fall back to whole-frame invalidation.
...
- wire more partial dirty-rectangle producers into the Rust host beyond focus transitions, view lifecycle changes, view-rect updates, and modal attach/leave, and refine resize beyond whole-frame invalidation
