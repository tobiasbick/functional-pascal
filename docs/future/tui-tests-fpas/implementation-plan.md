...
> **Next task:** Phase 8, Task **8.2** (document native TUI test API in `tui-app.md`).
...
- [x] **4.1 `Application.QueryRootViews(App): array of ViewId`.** From `ViewRegistry` roots (`crates/fpas-std/src/tui/view/`). **Verify:** create two root views, assert count/order.
...
- [x] **5.2 Bar item hover via `Move`.** Confirm/repair `Move` over a bar item sets `hovered` and paint reflects highlight colors. **Verify:** Rust unit test asserting hovered index + a `test_cell` color after a `Move`.
- [x] **5.3 Redraw determinism after pump.** Ensure `TestPump` flushes the back buffer so the next FPAS query sees post-event state (ties to 1.2). **Verify:** FPAS test: move → pump → `QueryScreenCell` shows highlight in the same program.
