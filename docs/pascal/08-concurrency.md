...
The compiler lowers `go` to dedicated VM opcodes (see Phase 1–2 in [`parallel-vm.md`](../future/parallel-vm.md)); the VM execute path for those opcodes is covered in Phase 6 of that layout (`fpas-vm` [`spawn.rs`](../../../crates/fpas-vm/src/vm/execute/concurrency/tasks/spawn.rs)):
