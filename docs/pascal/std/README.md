...
- `Std.Tui` dispatch mode (target) - [tui-app.md](tui-app.md) (`Application.Host*` → VM intrinsics **255**–**264**; `Application.HostRegisterOnExit` stores an `OnExit` callback for the future hosted run path; enum **`ExitReason`** is registered; **`Application.Run`** / runtime `OnExit` invocation are **not** lowered yet. See [tui-application-framework.md](../../future/tui-application-framework.md).)
