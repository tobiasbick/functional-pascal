...
- **Task failure / VM shutdown:** if a spawned task aborts with a runtime error, the runtime sets a **failure** path (not just “main finished” teardown) so other spawned work can stop; a waiter may see an execution-aborted diagnostic. Fix the fault in the spawned task. (VM: `SharedState::signal_runtime_failure` in `fpas-vm`, Phase 9 in [`parallel-vm.md`](../../rust/parallel-vm.md).)
