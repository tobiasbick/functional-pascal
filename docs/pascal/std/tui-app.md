...
Today, `[tui.md](tui.md)` documents `**Application.ReadEvent`**, `**ReadEventTimeout`**, `**PollEvent**`, and redraw helpers. The dispatch model replaces that pattern for full applications: the runtime owns the blocking loop and calls user `**On***` procedures. The poll-style entry points still coexist for programs that want manual event reads, but hosted `**Application.Run`** is the primary full-application surface.
