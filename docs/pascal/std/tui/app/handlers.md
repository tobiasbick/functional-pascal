...
Pascal can register hosted handlers in four complementary ways before `**Application.Run(App)`**:
...
**Required** for a minimal hosted run: either a global `**OnPaint`** with no retained roots registered before `**Run`**, or retained paint supplied by at least one local view paint handler or host widget view. In bundle form, `**ApplicationHandlers.OnPaint`** is optional; leave it unset for retained-view and widget applications.
...
| Slot           | Required | Role                                                                                                                            |
| -------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `OnPaint`      | no       | `Option of procedure(App: Application)` — full logical **frame** paint for global-paint applications with no retained roots. Use `None` or omit it when retained views/widgets paint the frame. |
...
- **Ordering:** `**OnExit`** runs **after** the last paint / input handler for that run; no further `**On*`** run after `**OnExit`** except what the implementation documents for catastrophic failure paths.
...
- `**OnPaint`:** Performs a **full logical frame** draw (entire buffer for the app) only for global-paint applications without retained roots. If any retained root exists before `**Application.Run`**, the run rejects global `**OnPaint`** as retained scene paint; register a root view paint handler or host widget views instead.
...
- **Relation to `RedrawPending`:** In dispatch mode, user code **typically does not poll** `**RedrawPending`**; the host consumes the pending flag when entering a hosted redraw. Global-paint applications invoke `**OnPaint`**; retained applications traverse damaged retained roots and invoke local paint/widget handlers.
...
- `**OnIdle`** must not block on input. It runs on the main VM thread under the same reentrancy rules
  as the other hosted-loop callbacks.
...
- **Ordering relative to other handlers:** `OnResize` events that arrive before the key in the host queue are coalesced and dispatched first; hosted redraw runs after input dispatch if a redraw is pending.
...
- **Ordering:** Resize is processed before any key event that arrived in the same batch; the host auto-requests a redraw when a resize fires, so hosted paint runs after `OnResize` returns (if no earlier redraw was already pending).
...
- **Ordering:** Mouse events are dispatched in arrival order, interleaved with key events. Hosted paint runs after input dispatch if a redraw was requested during the handler.
