...
| `TuiHostSetDesktopWorkArea`, `TuiHostCreateFrameView`, `TuiHostActivateNextWindow`, `TuiHostZoomFrameRoot`, `TuiHostRestoreFrameRoot`, `TuiQueryFrameRootState`, `TuiHostSetFrameContentSize`, `TuiHostScrollFrame`, `TuiHostSetFrameScrollOffset`, `TuiQueryFrameScrollState` | `Application`, geometry/title/`ViewId`, frame flags | Painted frame creation, desktop policy, window-manager actions, scrolling, and state queries. See [frames.md](frames.md). |
...
| `Application.HostSetDesktopWorkArea`, `HostCreateFrameView`, `ShowFramedDialog`, `HostActivateNextWindow`, `HostZoomFrameRoot`, `HostRestoreFrameRoot`, `QueryFrameRootState`, `HostCascadeFrameRoots`, `HostTileFrameRoots`, `HostSetFrameContentSize`, `HostScrollFrame`, `HostSetFrameScrollOffset`, `QueryFrameScrollState` | Matching frame-root `TuiHost*` / `TuiQuery*` intrinsic (**410..=422**) |
