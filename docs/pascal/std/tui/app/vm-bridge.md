...
| `TuiHostDispatchRedraw`       | `Application`                                    | If redraw is pending: runs registered global `OnPaint` only when no retained roots, view paint handlers, or host widgets exist; otherwise traverses retained paint handlers/widgets. Clears the flag with tag `6` when no eligible paint source exists. Pushes `integer`: `0` not pending, `5` paint ran, `6` cleared without handler.                                                                        |
...
| `TuiApplicationConfigure`     | `Application`, `ApplicationHandlers`             | Applies a bundled hosted-dispatch configuration. Replaces the current hosted handlers with the record fields from `ApplicationHandlers`; `OnPaint` and other handler fields use `Some(Handler)` or `None`, and `OnIdleMilliseconds <= 0` disables idle callbacks.        |
| `TuiApplicationRun`           | `Application`                                    | Hosted loop entrypoint. Requires either a global `OnPaint` handler with no retained roots registered before run, at least one local view paint handler, **or** at least one host widget view (`HostCreateSolidFillView`, `HostCreateMenuBarView`, or `HostCreateStatusBarView`). A retained root plus global `OnPaint` is rejected; register a root view paint handler or host widgets instead. The loop auto-requests the first redraw, blocks until `Application.HostRequestQuit(App)` is observed **or** the host stops the active run, records `ExitReason.UserQuit`, `ExitReason.HostStop`, `ExitReason.HostAndUserStop`, or `ExitReason.HostShutdown`, invokes `OnExit` when registered, and performs `Application.Close` semantics before returning. Pushes `()`. |
...
| `TuiApplicationShowFramedDialog` | `Application`, modal id, geometry, title, frame flags | Atomically creates an owned painted dialog frame, enters it modally, and pushes its `ViewId`. |
...
| `TuiHostSetDesktopWorkArea`, `TuiHostCreateFrameView`, `TuiHostActivateNextWindow`, `TuiHostZoomFrameRoot`, `TuiHostRestoreFrameRoot`, `TuiQueryFrameRootState`, `TuiHostCascadeFrameRoots`, `TuiHostTileFrameRoots`, `TuiQueryFrameWindowList`, `TuiHostActivateFrameWindow`, `TuiHostSetFrameContentSize`, `TuiHostScrollFrame`, `TuiHostSetFrameScrollOffset`, `TuiQueryFrameScrollState` | `Application`, geometry/title/`ViewId`, frame flags | Painted frame creation, desktop policy, window-manager actions, MDI window list, scrolling, and state queries. See [frames.md](frames.md). |
...
| `Application.HostSetDesktopWorkArea`, `HostCreateFrameView`, `ShowFramedDialog`, `HostActivateNextWindow`, `HostZoomFrameRoot`, `HostRestoreFrameRoot`, `QueryFrameRootState`, `HostCascadeFrameRoots`, `HostTileFrameRoots`, `QueryFrameWindowList`, `HostActivateFrameWindow`, `HostSetFrameContentSize`, `HostScrollFrame`, `HostSetFrameScrollOffset`, `QueryFrameScrollState` | Matching frame-root `TuiHost*` / `TuiQuery*` intrinsic (**410..=422**, **428..=429**) |
...
`Application.HostRegisterOnViewPaint(App, ViewId, OnViewPaint)` registers a local paint handler for one view. During retained hosted redraw, the host traverses retained roots depth-first. For each exposed damaged view it paints the native widget underlay, invokes the local handler, paints child subtrees, and then paints overlays. The callback receives local `Bounds` (`0, 0, width, height`); Console coordinates are relative to that view and writes are enforced against its effective clip. `Application.QueryViewRect` continues to return the absolute rectangle before clipping.

### Host widgets

Host widget views are painted entirely in Rust and satisfy the `Application.Run` paint prerequisite on their own. When the visible frame comes from retained views or widgets, omit `ApplicationHandlers.OnPaint` or set it to `None` (see `apps/ide/src/shell.fpas`).
