...
Turbo Vision bridge regression tests under `tests/tui/views/`, `tests/tui/smoke/`, `tests/tui/modals/`, and `tests/tui/events/`. IDE shell: `apps/ide/tests/`.
...
Interactive examples (manual terminal): `examples/pascal/tui/turbo_vision_dialog.fpas`, `turbo_vision_window.fpas`, `apps/ide` (Help → About during `Run`).
