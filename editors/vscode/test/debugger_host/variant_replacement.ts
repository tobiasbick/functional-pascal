/** Real Extension Host coverage for replacing complete enum and wrapper values. */
...
export async function verifyVariantReplacement(
  workspaceRoot: string,
  received: DapMessage[],
  sent: DapMessage[]
): Promise<void> {
  const lines = [
    "program DebuggerVariantReplacement;",
    "",
    "uses Std.Console;",
    "",
    "type",
  ...
  try {
    session = await startSession({
      type: "fpas",
      request: "launch",
      name: "FPAS debugger variant replacement",
      program: sourcePath,
      cwd: workspaceRoot,
      stopOnEntry: false
    });
    await waitFor(
      ...
      "variant-replacement breakpoint"
    );
    ...
    assert.equal(
      selectedFields.variables.find((field) => field.name === "Left")?.value,
      "10"
    );
    await waitFor(
      () => eventCount(sent.slice(marker.sent), "invalidated") >= 3,
      "variant replacement invalidation events"
    );
    const invalidations = eventCount(sent.slice(marker.sent), "invalidated");
    await assert.rejects(
      async () => setExpression(session as vscode.DebugSession, "Selected", "Pair(1, 2)", "Choice.Pair"),
      /callable|unknown|constructor/i
    );
    await assert.rejects(
      async () => setLocal(session as vscode.DebugSession, "Fixed", "Choice.Empty", "Choice.Empty"),
      /not mutable/i
    );
    assert.equal(
      eventCount(sent.slice(marker.sent), "invalidated"),
      invalidations,
      "rejected variant replacements emit no invalidation"
    );

    await session.customRequest("continue", { threadId: 1 });
    await waitFor(
      () => sent.slice(marker.sent).some((message) => message.event === "terminated"),
      "variant-replacement termination"
    );
    const output = sent
      .slice(marker.sent)
    ...
    assert.equal(
      output,
      "30\nfail\n0\n9\n",
      `continued debuggee observes variant replacements: ${JSON.stringify(
    ...
    assert.ok(
      ...
      "Extension Host forwards handle-based variant replacements"
    );
    assert.ok(
      received.slice(marker.received).some((message) => message.command === "setExpression"),
      "Extension Host forwards textual variant replacements"
    );
  } finally {
